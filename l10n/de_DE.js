OC.L10N.register(
    "welcome",
    {
    "Welcome" : "Willkommen",
    "Dashboard widget to display welcome content" : "Dashboard-Widget zum Anzeigen von Willkommensinhalten",
    "ℹ A Markdown rendering Dashboard widget to welcome all users.\n\n⚙ Configure via `Settings > Administration > Theming > Welcome widget`\n\n📄 Pick a Markdown document to be rendered in the widget\n\n💡 If no document is chosen, the widget won't be shown\n\n📝 Edit the Markdown document to update the widget in real-time\n\n🖼 Images are also supported\n\n💬 Configure a contact person to directly start a chat with (requires [Nextcloud Talk](https://apps.nextcloud.com/apps/spreed) to be installed)" : "ℹ Ein Markdown-Rendering-Dashboard-Widget zur Begrüßung aller Benutzer.\n\n⚙ Konfigurieren Sie über „Einstellungen > Verwaltung > Design > Willkommens-Widget“.\n\n📄 Wählen Sie ein Markdown-Dokument aus, das im Widget gerendert werden soll\n\n💡 Wenn kein Dokument ausgewählt ist, wird das Widget nicht angezeigt\n\n📝 Bearbeiten Sie das Markdown-Dokument, um das Widget in Echtzeit zu aktualisieren\n\n🖼 Bilder werden ebenfalls unterstützt\n\n💬 Konfigurieren Sie eine Kontaktperson, mit der Sie direkt einen Chat starten können (erfordert die Installation von [Nextcloud Talk] (https://apps.nextcloud.com/apps/spreed))",
    "Welcome widget" : "Willkommens-Widget",
    "The dashboard welcome widget will be displayed for all users only if you choose a markdown file." : "Das Willkommens-Widget des Dashboards wird nur dann allen Benutzern angezeigt, wenn Sie eine Markdown-Datei auswählen.",
    "Markdown content file" : "Markdown-Inhaltsdatei",
    "No file" : "Keine Datei",
    "Widget title" : "Widget-Titel",
    "Support contact" : "Support-Kontakt",
    "Choose a support user" : "Einen Support-Benutzer auswählen",
    "No recommendations. Start typing." : "Keine Empfehlungen. Beginnen Sie mit der Eingabe.",
    "No result." : "Kein Ergebnis.",
    "Support text" : "Support-Text",
    "Example: Call {name} to get help." : "Beispiel: Rufen Sie {name} an, um Hilfe zu erhalten.",
    "{name} will be replaced by the support user name" : "{name} wird durch den Support-Benutzernamen ersetzt",
    "Welcome admin options saved" : "Willkommens-Admin-Optionen gespeichert",
    "Failed to save welcome admin options" : "Willkommens-Admin-Optionen konnten nicht gespeichert werden",
    "Choose markdown welcome content file" : "Markdown-Willkommensinhaltsdatei auswählen",
    "No welcome content" : "Kein Willkommensinhalt",
    "Talk to your support contact ({name})" : "Sprechen Sie mit Ihrem Support-Kontakt ({name})",
    "Talk to {name}" : "Sprechen Sie mit {name}"
},
"nplurals=2; plural=(n != 1);");
